...
Este projeto integra **Python**, **Google Gemini (via google-genai)**, **Streamlit** e boas práticas de **UX** para criar uma experiência digital acessível e didática.
...
LLM	                           Modelo de linguagem Google Gemini via google-genai.
Base de Conhecimento           Arquivos JSON/CSV mockados contendo perfis, produtos e histórico de transações.
...
- Contexto dinâmico: utiliza dados mockados para personalizar respostas.
...
- Definir casos de uso prioritários (simulações, FAQs).

- Criar base de conhecimento mockada (JSON/CSV).

- Implementar módulo de cálculos financeiros em Python.

- Integrar LLM Google Gemini com Streamlit.

- Adicionar melhorias de contexto dinâmico.

- Implementar validação anti-alucinação.

- Testar métricas com usuários simulados.

- Iterar e ajustar conforme feedback.